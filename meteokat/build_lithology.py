...
Output 1 (the app data file): build/litho_grid.json   (gitignored staging dir)
        Generated here, then copied into the CLIENT repo as
        public/logic/litho_grid.json (this is a one-off rebuild, not a daily
        job — see README).
...
# Rebuilt rarely; written to a gitignored staging dir and copied into the
# client repo by hand (this repo does not own the app's public/ tree).
DEFAULT_OUT = REPO / "build" / "litho_grid.json"
