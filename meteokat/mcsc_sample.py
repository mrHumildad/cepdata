...
For every station in config/stations.json (server-local copy of the client's
src/logic/stations.json), queries the ICGC WMS
...
Output (build/forest_types.json — gitignored staging, copied into the CLIENT
repo as public/logic/forest_types.json; a one-off rebuild, not a daily job):
    { "<codi>": { "mcscClass": "221",
...
DEFAULT_STATIONS = SCRIPT_DIR.parent / "config" / "stations.json"
DEFAULT_OUT = SCRIPT_DIR.parent / "build" / "forest_types.json"
